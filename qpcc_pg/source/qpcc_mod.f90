...
subroutine qpcc_global(m, n, delta, image, snode, iter, info)
	implicit none
	...
	integer :: m, n, iter, info, step, nn, mm, mm_eq, step2, mn,&
		...
		ind(3, m * n * (n - 1) / 2), s, const, v(m), from, to, size, rank
	logical :: prob(m * n * (n - 1) / 2 + 1 - m, m * n * (n - 1)),&
		p(m * n * (n - 1)), snode(m * n * (n - 1)), feasible, next(2),&
		p_s(m * n * (n - 1))
