...
! November 12, 2014
!
! This driver demonstrates the usage of procedure "qpcc_local". The
! procedure is defined in module "qpcc" (qpcc_mod.f90).
!
program qpcc_local_dr
	use qpcc
	implicit none
	character(len = string_length) :: data_name, string
	double precision, allocatable :: delta(:, :), image(:, :)
	double precision :: re
	integer :: m, n, i, j, k, nran, iter, info, values0(8), values1(8)
	...
	call qpcc_local(m, n, delta, nran, image, info)
	...
	re = sqrt( sum( (/( ((&
		...
		j = (i + 1), n), i = 1, (n - 1) )/) ) )
	write(string, '("random image (", I0, " x ", I0, ")")') m, n
